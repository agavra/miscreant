// Adapted from SlateDB `src/cached_object_store/storage_fs.rs` (Apache-2.0). See
// the module comment in `mod.rs` for provenance.
//
// Adaptations: SlateDB's `SystemClock`/`DbRand` abstractions are replaced with
// `chrono::Utc::now()` / `tokio::time::sleep` and the `rand` crate directly;
// stats route through the `metrics` facade (see `stats.rs`); `format_bytes_si`
// is inlined here; import paths point at this crate's module.

// Vendored code keeps upstream style; allow the lints that would otherwise
// force it to diverge from the upstream source.
...
use super::storage::{LocalCacheEntry, LocalCacheHead, LocalCacheStorage};
...
impl FsCacheEntry {
    async fn atomic_write(&self, path: std::path::PathBuf, buf: Bytes) -> object_store::Result<()> {
        let tmp_path = path.with_extension(format!("_tmp{}", make_rand_suffix()));
...
impl LocalCacheEntry for FsCacheEntry {
    ...
    #[cfg(test)]
    async fn cached_parts(&self) -> object_store::Result<Vec<super::storage::PartID>> {
            ...
            match rx.recv().await {
                Some((path, access)) => match access {
                    EntryAccess::Read(bytes) => {
                        inner
                            .track_entry_accessed(path, bytes, Utc::now(), false)
                            .await;
                    }
                    EntryAccess::Write(bytes) => {
                        inner
                            .track_entry_accessed(path, bytes, Utc::now(), true)
                            .await;
                    }
                    EntryAccess::Delete => {
...
impl FsCacheEvictorInner {
    ...
    async fn track_entry_accessed(
        ...
        self.stats.object_store_cache_keys.set(entry_count as f64);
        self.stats
            .object_store_cache_bytes
            .set(self.cache_size_bytes.load(Ordering::Relaxed) as f64);
...
/// Human-readable byte size for log messages. Inlined from SlateDB's
/// `utils::format_bytes_si` (Apache-2.0).
...
mod tests {
    use super::*;
    use crate::storage::part_cache::test_util::gen_rand_bytes;
    use std::{io::Write, sync::atomic::Ordering, time::SystemTime};

    fn new_stats() -> Arc<CachedObjectStoreStats> {
        Arc::new(CachedObjectStoreStats::new())
    }
    ...
    #[tokio::test]
    async fn should_evict_when_over_budget() {
        // given: an evictor with a 2 KiB budget
        let temp_dir = tempfile::Builder::new()
            .prefix("part_cache_test_evictor_")
            .tempdir()
            .unwrap();
        ...
        // when/then: the first two 1 KiB files fit
        let path0 = gen_rand_file(temp_dir.path(), "file0", 1024);
        let evicted = evictor
            .track_entry_accessed(path0, 1024, Utc::now(), true)
            .await;
        assert_eq!(evicted, 0);

        let path1 = gen_rand_file(temp_dir.path(), "file1", 1024);
        let evicted = evictor
            .track_entry_accessed(path1, 1024, Utc::now(), true)
            .await;
        assert_eq!(evicted, 0);

        // when: a third file pushes over budget
        let path2 = gen_rand_file(temp_dir.path(), "file2", 1024);
        let evicted = evictor
            .track_entry_accessed(path2, 1024, Utc::now(), true)
            .await;

        // then: eviction brings the directory back under budget
    ...
    #[tokio::test]
    async fn should_backpressure_when_evictor_queue_is_full() {
        // given: a started evictor whose receiver is not draining
        let temp_dir = tempfile::Builder::new()
            .prefix("part_cache_test_evictor_backpressure_")
            .tempdir()
            .unwrap();
        ...
        // when: filling the channel to capacity is accepted
        for idx in 0..100 {
        ...
        // then: the next write is dropped rather than blocking the caller
        let accepted = evictor
    ...
    #[tokio::test]
    async fn should_pick_the_older_file_for_eviction() {
        // given: two files where file0 has the older access time
        let temp_dir = tempfile::Builder::new()
            .prefix("part_cache_test_evictor_")
            .tempdir()
            .unwrap();
        ...
        // Set file0's access time to the epoch so it is the oldest.
        let file = std::fs::File::options().write(true).open(&path0).unwrap();
        ...
        // then: the older file0 is chosen
    ...
    #[tokio::test]
    async fn should_keep_cache_size_stable_across_rescans() {
        // given
        let temp_dir = tempfile::Builder::new()
            .prefix("part_cache_test_evictor_")
            .tempdir()
        ...
        // then: the cache size is unchanged
        assert_eq!(cache_size_bytes, 2049);
    }

    #[tokio::test]
    async fn should_pick_a_random_available_index() {
        // given/when/then: a table of (keys, picked, exclude, expected-possible)
        struct Case {
            key_indices: &'static [usize],
            picked_indices: &'static [usize],
            exclude_idx: Option<usize>,
            expected_possible: &'static [usize],
        }
        let cases = [
            // 2 keys, nothing picked, no exclusion
            Case {
                key_indices: &[0, 1],
                picked_indices: &[],
                exclude_idx: None,
                expected_possible: &[0, 1],
            },
            // 2 keys, nothing picked, exclude index 0
            Case {
                key_indices: &[0, 1],
                picked_indices: &[],
                exclude_idx: Some(0),
            ...
            // 2 keys, nothing picked, exclude index 1
            Case {
                key_indices: &[0, 1],
                picked_indices: &[],
                exclude_idx: Some(1),
            ...
            // exclude_idx already in picked (redundant exclusion)
            Case {
                key_indices: &[0, 1],
                picked_indices: &[0],
                exclude_idx: Some(0),
            ...
            // no available index (all picked or excluded)
            Case {
                key_indices: &[0, 1],
                picked_indices: &[0],
                exclude_idx: Some(1),
        ...
        for case in cases.iter() {
            let keys: Vec<std::path::PathBuf> = case
                .key_indices
                .iter()
                .map(|i| std::path::PathBuf::from(format!("file{}", i)))
                .collect();
            let picked: HashSet<usize> = case.picked_indices.iter().copied().collect();
            let mut rng = rand::rng();

            if case.expected_possible.is_empty() {
                let result =
                    evictor.pick_random_available_index(&mut rng, &keys, &picked, case.exclude_idx);
                assert!(result.is_none(), "expected None, got {result:?}");
            } else {
                for _ in 0..100 {
                    let result = evictor.pick_random_available_index(
                        &mut rng,
                        &keys,
                        &picked,
                        case.exclude_idx,
                    ...
                    assert!(
                        result.is_some_and(|r| case.expected_possible.contains(&r)),
                        "returned {:?}, expected one of {:?}",
                        result,
                        case.expected_possible
                    );
                }
            }
        }
